...
# Jack Linden  #3704293
# Q learning flappy bird game
# I do not claim ownership of the flappy bird code,
# just the modifications to support the Q learning 
...
import pickle
from q_learner import State, Agent, ActionSet
import copy
...
class Bird(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, msec_to_climb, images=None):
...
def main():
    ...
    while True:
        ...
        while not done:
            ...
            if (_px+PipePair.WIDTH) - bird.x < 0:
                _px = pipes[1].x
            
            #Only runs on first iteration of game
            if old_state == None:
                x_dist = (_px+PipePair.WIDTH) - bird.x
                y_dist = _py - bird.y
                _px = pipes[0].x
                _py = (WIN_HEIGHT-pipes[0].bottom_height_px)
                x_dist = (_px+PipePair.WIDTH) - bird.x
                y_dist = _py - bird.y
                old_state = State(x_dist,y_dist,done,bird.msec_to_climb>0,_py)
            
            #Get action
            action = agent.decide(old_state,bird)    
            ...
            y_dist = _py - bird.y
            #Create new state
            new_state = State(x_dist,y_dist,done,bird.msec_to_climb > 0,_py )
            #If new state is different from the old state, call learning function
            if new_state != old_state:            
                agent.learn(old_state,new_state,action,pipe_cleared,pipe_collision)  
			#Set old state to be the new state
            old_state = new_state
            if pipe_cleared:
                up.add(_py)
                pipe_cleared = False
            ...
            display_surface.blit(score_surface, (pipes[0].x, PipePair.PIECE_HEIGHT))

            pygame.display.flip()
            frame_clock += 1
        runs += 1
        print('unique states: ', len(agent.Q.history),'runs: ', runs, '  score: ', score,  'unique: ', len(up), 'max consec: ', max)
